...
{
	auto FantomSnakeHead = Sprite::create("snake_head.png");				//Фантомная голова - для избежания коллизий
...
void HelloWorld::SnakeMove(std::vector<Vector<Sprite*>>& SnakesSprites, Vector <Sprite*>& SeedSprites, int SnakeNumber, float MoveTime)
{
	...
	if (i == 5)					//Если i !=5 значит свободного места для хвоста нет cвободного места
	{
		...
		SnakesSprites.at(SnakeNumber).at(0)->setPosition(x, y);				//"Фантомная" часть змеии мгновенн занимает новую позицию. Поскольку змеи ищут точку для перемешения в цикле - это не даёт двум змеям столкнуться
